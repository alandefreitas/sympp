...
/* create a new TCC compilation context, compiles libtcc1 if missing and
 * configures with atcc_configure_state */
LIBTCCAPI TCCState *atcc_new();

/* build ar archives with tiny_libmaker. Returns 0 on success, another value on
 * failure */
...
/* set error/warning callback for every TCCState that will be configured with
 * atcc_configure */
LIBTCCAPI void atcc_set_error_func(void *error_opaque, TCCErrorFunc error_func);
/* set include path of every TCCState that will be configured with
 * atcc_configure */
...
/* set path where the source files of libtcc1 are located, pass NULL to reset
 * this to default */
LIBTCCAPI void atcc_set_libtcc1_src_path(const char *path);
/* set path where the object files of libtcc1 should be generated, pass NULL to
