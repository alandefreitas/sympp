...
DLL_EXPORT void *__bound_memalign(size_t size, size_t align,
                                  const void *caller);
...
void __bound_checking(int no_check) { fetch_and_add(&no_checking, no_check); }
...
#define bound_warning(...) tcc_backtrace("^bcheck.c^BCHECK: " __VA_ARGS__)
...
void __bound_init(size_t *p) {
    ...
    tree =
        splay_insert((size_t)__ctype_b_loc(), sizeof(unsigned short *), tree);
    tree = splay_insert((size_t)(*__ctype_b_loc() - 128),
                        384 * sizeof(unsigned short), tree);
    tree =
        splay_insert((size_t)__ctype_tolower_loc(), sizeof(__int32_t *), tree);
    tree = splay_insert((size_t)(*__ctype_tolower_loc() - 128),
                        384 * sizeof(__int32_t), tree);
    tree =
        splay_insert((size_t)__ctype_toupper_loc(), sizeof(__int32_t *), tree);
    ...
    while (p[0] != 0) {
        ...
        if (print_calls) {
            dprintf(stderr, "%s, %s(): static var %p 0x%lx\n", __FILE__,
                    __FUNCTION__, (void *)p[0], (unsigned long)p[1]);
        }
#endif
        p += 2;
    }
...
#if HAS_ENVIRON
    {
        ...
        if (print_calls) {
            ...
            dprintf(stderr, "%s, %s(): environ %p 0x%lx\n", __FILE__,
                    __FUNCTION__, start, (unsigned long)((char *)p - start));
        }
#endif
    }
#endif
        ...
        while (tree) {
            if (print_heap && tree->type != 0)
                fprintf(stderr, "%s, %s(): %s found size %lu\n", __FILE__,
                        __FUNCTION__, alloc_type[tree->type],
    ...
    if (malloc_redir == NULL) {
        ...
        if (malloc_redir == NULL) {
            ...
            dprintf(stderr, "%s, %s(): initial %p, 0x%lx\n", __FILE__,
                    __FUNCTION__, ptr, (unsigned long)size);
            return ptr;
        }
    }
#endif
...
static void __bound_check(const void *p, size_t size, const char *function) {
    if (no_checking == 0 && size != 0 &&
        ...
        bound_error("invalid pointer %p, size 0x%lx in %s", p,
                    (unsigned long)size, function);
    }
}

static int check_overlap(const void *p1, size_t n1, const void *p2, size_t n2,
                         const char *function) {
...
void splay_printtree(Tree *t, int d) {
    ...
    fprintf(stderr, "%p(0x%lx:%u:%u)\n", (void *)t->start,
            (unsigned long)t->size, (unsigned)t->type, (unsigned)t->is_invalid);
