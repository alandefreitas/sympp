...
char *atcc_concatenate_path(const char *path, const char *filename,
    ...
    // create new string
    concatenated = (char *)tcc_realloc(
        fpath,
        (fpath_length + filename_length + extension_length + 1) * sizeof(char));
    memcpy(concatenated + fpath_length, filename, filename_length);
    if (extension_length > 0) {
        concatenated[fpath_length + filename_length] = '.';
        memcpy(concatenated + fpath_length + filename_length + 1, extension,
               extension_length - 1);
    }
...
char **atcc_split_string(const char *string, char delimiter) {
    ...
    if (string[size - 1] != delimiter)
        ...
        ++skippable_delimiters;
    tmp_string =
        (char *)tcc_malloc((size - skippable_delimiters + 1) * sizeof(char));
    ...
    for (i = 0; i < size + 1; ++i) {
        ...
        if (current_char == delimiter || current_char == '\0') {
            if (last_char != delimiter) {
                ...
                memcpy(tmp_string + dest_offset, string + src_offset,
                       segment_size);
