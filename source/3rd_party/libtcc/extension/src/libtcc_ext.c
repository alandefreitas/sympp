...
static ATccExtensionVariables aTccExtensionVariables = {NULL,
                                                        NULL,
                                                        TCC_LIBTCC1,
                                                        ALIBTCC1_SRC_PATH,
                                                        ALIBTCC1_OBJ_PATH,
...
void atcc_configure_state(TCCState *state) {
    if (state != NULL) {
        if (aTccExtensionVariables.error_function != NULL)
            tcc_set_error_func(state, aTccExtensionVariables.error_opaque,
                               aTccExtensionVariables.error_function);

        if (aTccExtensionVariables.libtcc1_dest_path != NULL) {
#ifdef _WIN32
            tcc_add_library_path(state,
                                 aTccExtensionVariables.libtcc1_dest_path);
#else
...
TCCState *atcc_new() {
    ...
    libtcc1_path =
        (char *)tcc_malloc((strlen(aTccExtensionVariables.libtcc1_dest_path) +
                            strlen(aTccExtensionVariables.libtcc1_name) + 1) *
                           sizeof(char));
...
int atcc_build_libtcc1() {
    ...
    result =
        atcc_create_dir_recursive(aTccExtensionVariables.libtcc1_dest_path);
    if (result == 0 && strcmp(aTccExtensionVariables.libtcc1_dest_path,
                              aTccExtensionVariables.libtcc1_obj_path) != 0)
    ...
    for (i = 0; i < file_count; i++) {
        ...
        file_name =
            atcc_concatenate_path(aTccExtensionVariables.libtcc1_src_path,
                                  splitted_file_name[0], splitted_file_name[1]);
        ...
        file_name =
            atcc_concatenate_path(aTccExtensionVariables.libtcc1_obj_path,
                                  splitted_file_name[0], "o");
    ...
    file_name = atcc_concatenate_path(aTccExtensionVariables.libtcc1_dest_path,
                                      splitted_name[0], "a");
...
void atcc_set_include_path(const char *path) {
    aTccExtensionVariables.include_path =
        (path == NULL) ? ALIBTCC_INCLUDE_PATH : path;
}
...
void atcc_set_libtcc1_src_path(const char *path) {
    aTccExtensionVariables.libtcc1_src_path =
        (path == NULL) ? ALIBTCC1_SRC_PATH : path;
}
void atcc_set_libtcc1_obj_path(const char *path) {
    aTccExtensionVariables.libtcc1_obj_path =
        (path == NULL) ? ALIBTCC1_OBJ_PATH : path;
}
void atcc_set_libtcc1_dest_path(const char *path) {
    aTccExtensionVariables.libtcc1_dest_path =
        (path == NULL) ? ALIBTCC1_DEST_PATH : path;
}
