...
/* returns 0 if file was removed successfully (or hasn't existed) or non zero
 * value if not */
int deleteLibtcc1() {
...
GTEST_TEST(Libtcc_Extension_Tests, compile2) {
    ...
    int (*func)(int, int);
    func =
        reinterpret_cast<int (*)(int, int)>(tcc_get_symbol(tccState, "test"));
    ASSERT_TRUE(func);
    ...
    // with MSVC you get an exception with error code 0xc0000005 (access
    // violation) when you call a function after tcc_delete even if relocated
    // (and therefore copied) to extra buffer (in this case codeBuffer).
#ifndef _MSC_VER
    ASSERT_EQ(func(3, 3), 216);
    ASSERT_EQ(func(2, 3), 125);
    ASSERT_EQ(func(2010, 1), 2011);
...
GTEST_TEST(Libtcc_Extension_Tests, compile3) {
    ...
    func = reinterpret_cast<char *(*)(const char *)>(
        tcc_get_symbol(tccState, "test"));
    ASSERT_TRUE(func);
