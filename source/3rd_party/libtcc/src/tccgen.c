    ...
    while (tok != ')') {
        ...
        case TOK_CLEANUP2: {
            ...
            if (!s) {
                ...
                tcc_error("'%s' is not declared as function",
                          get_tok_str(tok, &tokc));
            }
            ad->cleanup_func = s;
            next();
    ...
    if (tok == '?') {
        ...
        if (1) {
            ...
            if (bt1 == VT_VOID || bt2 == VT_VOID) {
                type.t = VT_VOID; /* NOTE: as an extension, we accept void on
                                     only one side */
            ...
            /* If one is a null ptr constant the result type
                ...
                if (type1.ref != type2.ref) {
                    type_incompatibility_error(
                        &type1, &type2,
                        "different struct/union types '%s' vs. '%s'");
                } else {
                ...
                type_incompatibility_error(&type1, &type2,
                                           "type '%s' is incompatible to '%s'");
            }

            /* keep structs lvalue by transforming `(expr ? a : b)` to `*(expr ?
               &a : &b)` so that `(expr ? a : b).mem` does not error  with
               "lvalue expected" */
...
static void expr_eq(void) {
    ...
    expr_cond();
    if ((t = tok) == '=' || (t >= TOK_A_MOD && t <= TOK_A_DIV) ||
        t == TOK_A_XOR || t == TOK_A_OR || t == TOK_A_SHL || t == TOK_A_SAR) {
        test_lvalue();
        next();
        if (t == '=') {
            expr_eq();
