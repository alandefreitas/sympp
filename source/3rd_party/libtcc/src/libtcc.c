...
ST_FUNC AFileHandle atcc_open_file_handle(const char *filename, int flags) {
    ...
    if (fh.fd < 0 && asset_manager != NULL)
        fh.asset =
            AAssetManager_open(asset_manager, filename, AASSET_MODE_UNKNOWN);
    else
        fh.asset = NULL;
#endif
    return fh;
...
static AFileHandle _tcc_open(TCCState *s1, const char *filename) {
    ...
    if ((s1->verbose == 2 && atcc_file_handle_is_valid(fh)) || s1->verbose == 3)
        printf("%s %*s%s\n", atcc_file_handle_is_valid(fh) ? "->" : "nf",
...
static int tcc_compile(TCCState *s1, int filetype, const char *str,
                       AFileHandle fh) {
