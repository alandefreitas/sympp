...
#ifndef SYMPP_BOOLEAN_H
#define SYMPP_BOOLEAN_H

#include <sympp/node/terminal/number_interface.h>

namespace sympp {

    class boolean : public number_interface {
      public:
        boolean();
        boolean(const boolean &);
        boolean(boolean &&) noexcept;
        explicit boolean(bool);
        explicit boolean(int);
        explicit boolean(double);
        explicit boolean(const number_interface &);
        explicit boolean(const node_interface &);
        explicit boolean(const sym &);
        ~boolean() override;
